...
See the sample JSON files in `docs/contracts/samples/` for implementation-ready payloads. Use `fixtures/golden/taxonomy-v1.json` as the canonical modeled reference when validating default segment-owned layout plus Species → Crop Type → Cultivar relationships for imports, demos, and documentation.
